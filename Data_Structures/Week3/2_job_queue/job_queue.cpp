...
void start(const vector<long long int>& jobs_list, vector<Threads>& mythreads) {
    ...
    for (int i = 0; i < jobs_list.size(); i++) {
        ...
        if (jobs_list[i] != 0)
            ChangePriority(0, mythreads[0].start_time + jobs_list[i], mythreads);
    }
...
int main() {
    ...
    vector<long long int> jobs_list(m);
    for (int i = 0; i < m; i++)
        cin >> jobs_list[i];

    vector<Threads> mythreads(n);
    for (int i = 0; i < n; i++)
